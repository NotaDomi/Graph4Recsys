...
textual_embeddings_folder = './data/datasets/textual_embeddings_05/sentence_transformers/sentence-transformers/all-mpnet-base-v2/1'

#visual_embeddings_folder_indexed = 'visual_embeddings_final_indexed/torch/ResNet50/avgpool'
textual_embeddings_folder_indexed = './data/datasets/textual_embeddings_05/sentence_transformers/sentence-transformers/all-mpnet-base-v2/2'

train = pd.read_csv('./data/Office_Products/train_final.tsv', sep='\t', header=None)
val = pd.read_csv('./data/Office_Products/val_final.tsv', sep='\t', header=None)
test = pd.read_csv('./data/Office_Products/test_final.tsv', sep='\t', header=None)
...
#/content/Graph-Missing-Modalities/data/datasets/textual_embeddings_05/sentence_transformers/sentence-transformers/all-mpnet-base-v2/1/('A00473363TJ8YSZ3YAGG9', 'B00007E7D2').npy
for key, value in items_map.items():
    for key2, value2 in users_map.items():
        try:
            #np.save(f'{visual_embeddings_folder_indexed}/{value}.npy', np.load(f'{visual_embeddings_folder}/{key}.npy'))
            np.save(f'{textual_embeddings_folder_indexed}/{value2},{value}.npy', np.load(f'{textual_embeddings_folder}/(\'{key2}\', \'{key}\').npy'))
